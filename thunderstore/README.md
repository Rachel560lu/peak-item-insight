...
按 **Esc → Item Insight 设置 → 物品简介样式** 选择 **极简 / 详细**。详细卡片显示名称旁的物品图标、已有的烹饪次数、效果数值、已知的持续时间或延迟、效果结束后的副作用，以及必要的使用／触发条件；没有的信息直接省略。精力条预览仍可独立关闭，选择 **Off** 即可只保留描述卡片。也可在 Config editor 中调整 **UI.DescriptionStyle**（Minimal / Detailed）。
